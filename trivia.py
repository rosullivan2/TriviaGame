...
def display_question(question):
    ...
    for option in question["options"]:
        print(option)
    print("---------------")  # Add a line of dashes after each question
...
def play_trivia_game(questions, lives=3):  # Add 'lives' with a default value of 3
    ...
    for question in questions:
        ...
        if lives > 0:  # Offer lifeline only if the player has remaining lives
            print("\nDo you want to use a lifeline? (Press '5' for 50/50)")
            choice = input("Enter your choice: ").strip()
            if choice.lower() == "5":
                print("---------------")
                print("Implementing 50/50 lifeline...")
                options_copy = question["options"][:]  # Create a copy of options list
                correct_option = question["options"][question["answer"] - 1]  # Get correct option
                options_copy.remove(correct_option)  # Remove correct option from copy
                wrong_option = random.choice(options_copy)  # Randomly choose one wrong option
